...
export function summarizeState(state) {
	...
	return {
		...
		mustReview: values.filter(
			(value) => (adjustedReviewSignals(value).adjustedReviewRisk || 0) >= 0.65
		).length,
		keyLogic: values.filter((value) => (adjustedReviewSignals(value).adjustedKeyLogic || 0) >= 0.65)
			.length,
