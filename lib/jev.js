...
import { adjustedReviewSignals } from './review.js';
...
export function buildQuestions(files) {
	...
	files.forEach((file, index) => {
		...
		questions[`${prefix}__validation_catch`] = {
			type: 'noul',
			instructions: `If a defect were introduced by the changed logic in ${reference}, is it likely to be caught before release by ordinary validation even without deep specialist review of this file? Consider type checking, linting, builds, automated tests, visual regression, routine manual review, and normal QA that are evidenced or reasonably implied by the changed content and repository context.`,
			criteria: {
				true: 'The likely failure is obvious, deterministic, visible, compile-time detectable, covered by nearby tests, or routinely exercised before release. Examples include most straightforward CSS/layout breakage under visual QA, type errors, build failures, and behavior with direct automated coverage.',
				false:
					'The failure could pass normal validation and surface only subtly or in production, such as silent data corruption, security or permission flaws, race conditions, untested edge cases, migration hazards, configuration drift, or environment-specific behavior.'
			}
		};
		questions[`${prefix}__database_risk`] = {
			type: 'noul',
			instructions: `Does the change in ${reference} introduce or modify a database schema, persisted data, or a data migration in a way that requires high-priority human review? Judge the changed content rather than relying only on the filename.`,
			criteria: {
				true: 'It creates, alters, renames, or drops a table, column, index, constraint, relationship, enum, or database type; changes nullability, defaults, uniqueness, or storage semantics; or performs a backfill, destructive update, delete, data conversion, or migration that could lose, corrupt, lock, or misinterpret persisted data. Additive tables and columns still qualify.',
				false:
					'It does not change database structure or persisted data. Merely reading or writing through an existing schema, changing application-only types, or editing tests, mocks, documentation, or styling does not qualify.'
			}
		};
	});
...
export function answersForFiles(files, response) {
	return Object.fromEntries(
		files.map((file, index) => {
			...
			Object.assign(value, adjustedReviewSignals(value));
